...
| `province_border` | Land neighbour with a different `province`. Off-map does **not** count. | 99.985% | 100.000% |
...
- **`province_border`** — a single cell, (104, 45). Three neighbours belong to a
  different province but only one bit is set. No rule we tried explains it;
  treated as an anomaly in the shipped data.
